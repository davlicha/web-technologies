...
export class LibraryManager {
    ...
    borrowBook(bookId: string, userId: string): { success: boolean, message: string, book?: IBook, user?: IUser } {
        const book = this.bookLibrary.find(bookId);
        if (!book) return { success: false, message: "Книгу не знайдено." };
        if (book.borrowedBy) return { success: false, message: "Книга вже позичена." };
        ...
        return { success: true, message: "Книгу успішно позичено!", book, user };
    }

    returnBook(bookId: string): { success: boolean, message: string, book?: IBook } {
        const book = this.bookLibrary.find(bookId);
        if (!book) return { success: false, message: "Книгу не знайдено." };
        if (!book.borrowedBy) return { success: false, message: "Книга не була позичена." };
    ...
    getBooks(): IBook[] { return this.bookLibrary.getAll(); }
    getUsers(): IUser[] { return this.userLibrary.getAll(); }
    addBook(book: IBook) { this.bookLibrary.add(book); }
    addUser(user: IUser) { this.userLibrary.add(user); }
    removeBook(id: string) { this.bookLibrary.remove(id); }
    removeUser(id: string) { this.userLibrary.remove(id); }
}
