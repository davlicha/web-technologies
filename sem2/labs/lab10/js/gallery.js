...
function renderUsers(users) {
    ...
    users.forEach(user => {
        const fullName = [user.name.first, user.name.last].join(' ');
...
function filterAndSortUsers() {
    ...
    users = users.filter(user => {
        const fullName = [user.name.first, user.name.last].join(' ').toLowerCase();
        const age = user.dob.age;
        const email = user.email
        if (showLikedOnly) {
            return (
    ...
    const gallery = document.querySelector(".person-gallery");
    if (users.length === 0) {
        gallery.innerHTML = `<p class="no-results">No results found. Try changing your filters.</p>`;
        return;
    }
